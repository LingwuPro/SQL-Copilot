...
class LayerNormWithCompactor(nn.LayerNorm):
    ...
    def __init__(self, 
        ...
        self.in_comp = Compactor(
            normalized_shape, bias=compactor_bias, mask_pos="no")
    ...
    def extract(self, indices: torch.Tensor) -> nn.LayerNorm:
        self.kw_args["normalized_shape"] = indices.shape[0]
        new_layernorm = nn.LayerNorm(**self.kw_args)
        new_layernorm.weight.copy_(self.weight[indices])
        new_layernorm.bias.copy_(self.bias[indices])
